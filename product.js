        ...
        function initCart() {
            ...
            if (savedUser && savedUser.email) {
                const perKey = localStorage.getItem(`beautyBloomCart_${savedUser.email}`);
                cart = perKey ? JSON.parse(perKey) : [];
            } else {
                cart = [];
            }
            updateCartCount();
        ...
        // Save cart to localStorage (per-user and shared copy)
        function saveCart() {
        ...
        // Listen for auth changes (signup/login/logout) made in `javascript.js` and reload cart/user state
